...
def log(log_type, msg=None, asset=None, new_asset=None, request=None):
    ...
    if log_type == 'upline':
        ...
        event.name = '%s <%s> :  %s!' % (asset.asset_type, asset.sn, _('data update'))
        event.asset = asset
        event.detail = _('update successfully')
    elif log_type == 'update_failed':
        event.name = '%s <%s> :  %s' % (asset.asset_type, asset.sn, _('update failed'))
        event.asset = asset
        event.detail = '%s!\n%s' % (_('update failed'), msg)
    # 更多日志类型.....
    event.save()
...
class ApproveAsset(object):
    ...
    def _server_upline(self):
        ...
        try:
            ...
            print(_('New Server upline'))  # 新服务器上线
            return True
    ...
    def _create_ram(self, asset):
        ...
        for ram_dict in ram_list:
            if not ram_dict.get('slot'):
                raise ValueError(_('Unknown RAM slot'))  # 使用虚拟机的时候，可能无法获取内存插槽，需要你修改此处的逻辑。
            ram = models.RAM()
            ram.asset = asset
    ...
    def _create_disk(self, asset):
        ...
        for disk_dict in disk_list:
            if not disk_dict.get('sn'):
                raise ValueError(_('Unknown serial number disk'))  # 根据sn确定具体某块硬盘。
            disk = models.Disk()
            disk.asset = asset
    ...
    def _create_nic(self, asset):
        ...
        for nic_dict in nic_list:
            if not nic_dict.get('mac'):
                raise ValueError(_('NIC is missing mac address'))
            if not nic_dict.get('model'):
                raise ValueError(_('Unknown NIC model'))
...
class UpdateAsset:
    ...
    def _server_update(self):
        try:
            ...
            print(_('Asset data has updated'))
