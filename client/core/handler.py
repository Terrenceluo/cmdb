...
class ArgvHandler(object):
    ...
    def report_data():
        ...
        print(u'正在将数据发送至： [%s]  ......' % url)
        try:
            # 使用Python内置的urllib.request库，发送post请求。
            # 需要先将数据进行封装，并转换成bytes类型
            data_encode = urlencode(data).encode('utf-8')
            response = urlopen(url=url, data=data_encode, timeout=settings.Params['request_timeout'])
            print(u'\033[31;1m发送完毕！\033[0m ')
            message = response.read().decode('utf-8')
            print(u'返回结果：%s' % message)
        except Exception as e:
            message = u'发送失败'
            print(u'\033[31;1m发送失败，%s\033[0m' % e)
        # 记录发送日志
        with open(settings.PATH, 'ab') as f:
            string = u'发送时间：%s \t 服务器地址：%s \t 返回结果：%s \n' % (time.strftime('%Y-%m-%d %H:%M:%S'), url, message)
